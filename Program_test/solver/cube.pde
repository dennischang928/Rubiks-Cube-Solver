...
class Cube {
  ...
  String different = " ";
  String Notation_Step_To_Get_Here[] = {"", "", "", "", "", ""};
  int Depth = 0;
  Cube() {
    for (int i  =0; i<6; i++) {
      Child[i] = new Face();
  ...
  Cube[] Create_New_Generation (String rotation_s[]) {

    Cube Generation[] = new Cube[Cube_Enable_Num[Depth+1]];
    Cube OUTPUT[] = new Cube[Cube_Enable_Num[Depth+1]];
    for (int i =0; i<Braching_Factor; i++) {
      Generation[i] = this.command(rotation_s[i]);
      Generation[i].enable = true;
      Generation[i].Depth = this.Depth + 1;
      different = rotation_s[i];
      for (int x = 0; x<6; x++) {
        Generation[i].Notation_Step_To_Get_Here[x] = this.Notation_Step_To_Get_Here[x];
      }
      Generation[i].Notation_Step_To_Get_Here[Generation[i].Depth] = rotation_s[i];
      OUTPUT[i] = Generation[i].Clone();
      //printArray(OUTPUT[i].Notation_Step_To_Get_Here);
      //println("----------");
    }
    return OUTPUT;
  }
  ...
  void SETUP() {
    ...
    for (int i =0; i<6; i++) {
      Notation_Step_To_Get_Here[i] = new String();
    }
  ...
  Cube Clone() {
    ...
    OUTPUT.Notation_Step_To_Get_Here = this.Notation_Step_To_Get_Here;
